...
if (isset($_POST['submit'])) {
	...
	$dch = "INSERT INTO `table_spp` (kode_spp, diskon, tanggal, tempo, keterangan, status, pph, ppn) VALUES ('".$kode."', '".$_POST['diskon']."', '".$_POST['tanggal']."', 0, '".$_POST['keterangan']."', 0, '".$_POST['valuetambahan0']."', '".$_POST['valuetambahan1']."')";
	// echo $dch;
	$query = mysqli_query($conn, $dch);
	$last_id = mysqli_insert_id($conn);
	for ($i=0; $i <= $number; $i++) {
		$dch = "INSERT INTO `table_spp_detail` (`id_spp_detail`, `id_produk`, `id_spp`, `harga`, `qty`, `total`, `kodesup`) VALUES (NULL, '".$_POST['id_produk'][$i]."', '".$last_id."', '".$_POST['harga'][$i]."', '".$_POST['qty'][$i]."', '".$_POST['total'][$i]."', '".$_POST['kodesuphid2'][$i]."')";
		// echo $dch;
		$query = mysqli_query($conn, $dch);

		$sql = "INSERT INTO `history_spp_detail` (`id_spp_detail`, `id_produk`, `id_spp`, `harga`, `qty`, `total`, `kodesup`) VALUES (NULL, '".$_POST['id_produk'][$i]."', '".$last_id."', '".$_POST['harga'][$i]."', '".$_POST['qty'][$i]."', '".$_POST['total'][$i]."', '".$_POST['kodesuphid2'][$i]."')";
		// echo $sql;
		mysqli_query($conn, $sql);
	}
	for ($i=0; $i <= $number2; $i++) {
		$sql = "INSERT INTO `tabel_spp_acccount` (`id_spp_account`, `kode_rekening`, `id_spp`, `nominal`, `kodesup`) VALUES (NULL, '".$_POST['koderekening'][$i]."', '".$last_id."', '".$_POST['nominal'][$i]."', '".$_POST['kodesuphid1'][$i]."')";
		mysqli_query($conn, $sql);
	}
	if($query){
		$jenis_alert = "success";
									...
									<table class="table bordered" id="dynamic_field" width="100%" cellspacing="0">
										...
										<tfoot>
											...
											while ($row = mysqli_fetch_array($query)) {
												?>
												<tr>
												<th colspan="5" style="text-align:right"><?php echo $row['nama_tbh']; ?></th>
											 ...
											 ?>
											<tr>
												<th colspan="5" style="text-align:right">Netto</th>
												<th id="net">0</th>
												<th></th>
											</tr>
										</tfoot>
...
<script>
	...
	$('#add2').click(function(){
		i2++;
		$('#dynamic_field2').append('<tr id="row'+i2+'"><td align="center"><input class="text-center" name="koderekening[]" id="kodes'+i2+'" style="width: 100%;border: none;" readonly></td><td align="center"><input type="text" id="nama'+i2+'" style="width: 100%;border: none;background: #fff;" list="browsers2" onchange="sjsj'+i2+'(value);"><datalist id="browsers2"><?php $rdcjn = mysqli_query($conn, "SELECT * FROM `tabel_master` where nama_rekening like 'Kas%' OR nama_rekening like 'Bank%'");while ($rdjc = mysqli_fetch_array($rdcjn)) { echo '<option value="'.$rdjc['kode_rekening']." ".$rdjc['nama_rekening'].'">'; } ?></datalist></td><td align="center"><input type="text" id="kodesup1'+i2+'" style="width: 100%;border: none;background: #fff;" list="browserss2" onchange="sjsjr'+i2+'(value);"><datalist id="browserss2"><?php $jdjd = mysqli_query($conn, "SELECT * FROM `supplier`");while ($dd = mysqli_fetch_array($jdjd)) { echo '<option value="'.$dd['Nama'].'">'; } ?></datalist><input type="text" id="kodesuphid1'+i2+'" name="kodesuphid1[]" hidden></td><td align="center"><input type="text" id="saldo'+i2+'" readonly="readonly" name="saldo[]" class="text-center" style="width: 100%;border: none;background: #fff;" ></td><td align="center"><input type="number" id="nominal'+i2+'" name="nominal[]" class="text-center totalsubnya" oninput="disk2'+i2+'()" style="width: 100%;border: none;background: #fff;" ></td><td align="center"><button type="button" name="remove" id="'+i2+'" class="btn btn-danger btn-sm btn_remove">hapus</button></td></tr>');
		c2=i2;
	});
	$('#add').click(function(){
		i++;
		$('#dynamic_field').append('<tr id="row'+i+'"><td align="center"><input class="text-center" style="width: 100%;border: none;background: #fff;" id="kode_produk'+i+'" name="kode_produk[]" type="text" readonly><input class="text-center" style="width: 100%;border: none;background: #fff;" id="kode'+i+'" name="id_produk[]" type="hidden" readonly></td><td><input onchange="produk'+i+'(value);" list="browsers" style="width: 100%;border: none;"><datalist id="browsers"><?php $rdcjn = mysqli_query($conn, "SELECT * FROM `produk`");while ($rdjc = mysqli_fetch_array($rdcjn)) { echo '<option value="'.$rdjc['nama_produk'].'">'; } ?></datalist></td><td><input class="text-center" style="width: 100%;border: none;background: #fff;" id="harga'+i+'" name="harga[]" type="text" readonly></td><td align="center"><input type="text" id="namas'+i+'" style="width: 100%;border: none;background: #fff;" list="browsers4" onchange="sjsjrs'+i+'(value);"><datalist id="browsers4"><?php $rdcjn = mysqli_query($conn, "SELECT * FROM `supplier`");while ($rdjc = mysqli_fetch_array($rdcjn)) { echo '<option value="'.$rdjc['Nama'].'">'; } ?></datalist><input type="text" name="kodesuphid2[]" id="kodesups'+i+'" hidden></td><td><input class="text-center qty" value="0" style="width: 100%;border: none;" type="number" id="qty'+i+'" name="qty[]" oninput="sum'+i+'();"></td><td><input class="text-center tot" style="width: 100%;border: none;background: #fff;" id="total'+i+'" name="total[]" type="text" readonly></td><td align="center"><button type="button" name="remove" id="'+i+'" class="btn btn-danger btn-sm btn_remove">hapus</button></td></tr>');
		c=i;
	});
	...
	for ($i=1; $i <= 1000; $i++) {
		...
		function sum<?php echo $i; ?>(value){
			...
			$('#net').text(convertToRupiah(tot+(tot*(pph/100))+(tot*(ppn/100))));
			button_check();
		}
